...
$oExportPdf = new Motel_Live_Data_Summary_export_pdf;
$oExportPdf->exportPdf();

class Motel_Live_Data_Summary_export_pdf {
...
<head>
 ...
 function checkPDF() {
  $.get(nm_url_rand("Motel_Live_Data_Summary_gauge_ctrl.php"), {
...
<?php
$NM_pdfbase = (isset($_SESSION['sc_session'][$aParams['script_case_init']]['Motel_Live_Data_Summary']['pdf_name'])) ? $_SESSION['sc_session'][$aParams['script_case_init']]['Motel_Live_Data_Summary']['pdf_name'] : 'sc_pdf_' . date('YmdHis') . '_' . rand(0, 1000) . '_Motel_Live_Data_Summary.pdf';
$NM_tit_doc = (isset($_SESSION['sc_session'][$aParams['script_case_init']]['Motel_Live_Data_Summary']['pdf_name'])) ? $_SESSION['sc_session'][$aParams['script_case_init']]['Motel_Live_Data_Summary']['pdf_name'] : "Motel_Live_Data_Summary.pdf";
$NM_pdfurl  = $STR_tmp;
$NM_target  = "_self";
$NM_volta   = (!isset($aParams['nmgp_opcao']) || 'pdf_res' != $aParams['nmgp_opcao']) ? "volta_grid" : "resumo";
?>
<form name="F0" method="post" action="Motel_Live_Data_Summary.php" target="<?php echo $NM_target; ?>" style="display: none"> 
