...
function checkTableName($shortTName, $type=false)
{
	...
	if ("Acumulative_Sales_By_Month" == $shortTName && ($type===false || ($type!==false && $type == 3)))
		return true;
	if ("Acumultive_Sale_by_Shift" == $shortTName && ($type===false || ($type!==false && $type == 3)))
		return true;
	if ("Top_Electricity_Consumption_Rooms" == $shortTName && ($type===false || ($type!==false && $type == 3)))
		return true;
	if ("Top_10_Rooms_Chart" == $shortTName && ($type===false || ($type!==false && $type == 3)))
		return true;
	return false;
}
...
{
	$arr = array();
		$arr[]="Acumulative Sales By Month";
		$arr[]="Acumultive Sale by Shift";
		$arr[]="Top Electricity Consumption Rooms";
		$arr[]="Top 10 Rooms Chart";
	return $arr;
}
...
{
	if($shorttable=="Acumulative_Sales_By_Month")
		return "2DColumn";
	if($shorttable=="Acumultive_Sale_by_Shift")
		return "2DColumn";
	if($shorttable=="Top_Electricity_Consumption_Rooms")
		return "Combined";
	if($shorttable=="Top_10_Rooms_Chart")
		return "2DColumn";
	return "";
}
...
function GetUserPermissions($table="")
{
	...
	if(!$table)
		$table = $strTableName;
	$permissions = "";
	if($table=="Acumulative Sales By Month")
	{
			$permissions =  "ADESPIM";
	}
	if($table=="Acumultive Sale by Shift")
	{
			$permissions =  "ADESPIM";
	}
	if($table=="Top Electricity Consumption Rooms")
	{
			$permissions =  "ADESPIM";
	}
	if($table=="Top 10 Rooms Chart")
	{
			$permissions =  "ADESPIM";
	}
