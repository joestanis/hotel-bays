...
if(!handle){if(!type||!fn||!el||!filter){Y.log("delegate requires type, callback, parent, & filter","warn");return;}
...
return ret;}};delegate.compileFilter=Y.cached(function(selector){return function(target,e){return selectorTest(target._node,selector,(e.currentTarget===e.target)?null:e.currentTarget._node);};});delegate._applyFilter=function(filter,args,ce){var e=args[0],container=ce.el,target=e.target||e.srcElement,match=[],isContainer=false;if(target.nodeType===3){target=target.parentNode;}
...
args.shift();return match;};Y.delegate=Y.Event.delegate=delegate;},'3.6.0',{requires:['node-base']});