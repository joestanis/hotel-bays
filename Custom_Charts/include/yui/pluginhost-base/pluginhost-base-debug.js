...
else{Y.log("Attempt to replug an already attached plugin, and we can't setAttrs, because it's not Attribute based: "+ns);}}else{this[ns]=new Plugin(config);this._plugins[ns]=Plugin;}}
...
return this;},hasPlugin:function(ns){return(this._plugins[ns]&&this[ns]);},_initPlugins:function(config){this._plugins=this._plugins||{};if(this._initConfigPlugins){this._initConfigPlugins(config);}},_destroyPlugins:function(){this.unplug();}};Y.namespace("Plugin").Host=PluginHost;},'3.6.0',{requires:['yui-base']});