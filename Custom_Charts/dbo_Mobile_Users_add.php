...
$layout = new TLayout("add2","PurificBlueWave","MobileBlueWave");
...
$layout->blocks["top"][] = "details";$page_layouts["dbo_Mobile_Users_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "dbo_Mobile_Users_inline_add.htm";
else
	$templatefile = "dbo_Mobile_Users_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: dbo_Mobile_Users_add.php');
...
//	processing login - start
	...
	if($inlineAddOption)
	{
		...
		$control_login->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'dbo_Mobile_Users_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'dbo_Mobile_Users_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: dbo_Mobile_Users_".$pageObject->getPageType().".php");
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("login_label","<label for=\"".GetInputElementId("login", $id, PAGE_ADD)."\">","</label>");
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "dbo.Mobile_Users";
		$options['firstTime'] = 1;
