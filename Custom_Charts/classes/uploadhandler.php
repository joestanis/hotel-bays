...
class UploadHandler
{
    ...
    public function get_file_object($file_name) {
        ...
        if (is_file($file_path) && $file_name[0] !== '.') {
            ...
            foreach($this->options['image_versions'] as $version => $options) {
				if ($file_path)
					$file["thumbnail_url"] = rawurlencode($file_path);				
            }
    ...
    public function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index) {
        ...
        if ($this->validate($uploaded_file, $file, $error, $size, $index, $uploadDir)) {
            ...
            $file["name"] = $this->tempnam_sfx($uploadDir, $path_parts["extension"]);
            ...
            if($this->options["resizeOnUpload"]){
            	...
            	$new_file_name = $this->tempnam_sfx($uploadDir, $path_parts["extension"]);
            ...
            if ($file_size === $file["size"]) {
                ...
                foreach($this->options['image_versions'] as $version => $options) {
					...
					$thumbnail_name = $options['thumbnailPrefix'].$this->tempnam_sfx($thumbUploadDir.$options['thumbnailPrefix'], 
    ...
    public function post() {
        ...
        foreach ($info as $file)
        {
        	...
        	$result[] = $this->buildUserFile($file);
        }
    ...
    public function tempnam_sfx($path, $suffix){ 
		do{ 
			$fileName = generatePassword(15).'.'.$suffix;
			$file = $path.$fileName; 
			$fp = @fopen($file, 'x'); 
		}while(!$fp); 
