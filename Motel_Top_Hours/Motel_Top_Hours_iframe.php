...
$oExportPdf = new Motel_Top_Hours_export_pdf;
$oExportPdf->exportPdf();

class Motel_Top_Hours_export_pdf {
...
<head>
 <title><?php echo $Nm_lang['lang_othr_grid_titl'] ?> - dbo.Motel_Top_Hours :: PDF</title>
 ...
 function checkPDF() {
  $.get(nm_url_rand("Motel_Top_Hours_gauge_ctrl.php"), {
...
<?php
$NM_pdfbase = (isset($_SESSION['sc_session'][$aParams['script_case_init']]['Motel_Top_Hours']['pdf_name'])) ? $_SESSION['sc_session'][$aParams['script_case_init']]['Motel_Top_Hours']['pdf_name'] : 'sc_pdf_' . date('YmdHis') . '_' . rand(0, 1000) . '_Motel_Top_Hours.pdf';
$NM_tit_doc = (isset($_SESSION['sc_session'][$aParams['script_case_init']]['Motel_Top_Hours']['pdf_name'])) ? $_SESSION['sc_session'][$aParams['script_case_init']]['Motel_Top_Hours']['pdf_name'] : "Motel_Top_Hours.pdf";
$NM_pdfurl  = $STR_tmp;
$NM_target  = "_self";
$NM_volta   = (!isset($aParams['nmgp_opcao']) || 'pdf_res' != $aParams['nmgp_opcao']) ? "volta_grid" : "resumo";
?>
<form name="F0" method="post" action="Motel_Top_Hours.php" target="<?php echo $NM_target; ?>" style="display: none"> 
...
{
?>
<iframe name="nmIfrPdf" height="1px" width="1px" frameborder="0" scrolling="no" allowtransparency="true" src="Motel_Top_Hours.php?nmgp_opcao=pdf&script_case_init=<?php echo $aParams['script_case_init'] ?>&script_case_session=<?php echo session_id() ?>&pbfile=<?php echo $aParams['pbfile'] ?>&pdf_base=<?php echo $NM_pdfbase ?>&pdf_url=<?php echo $NM_pdfurl ?>&sc_apbgcol=<?php echo urlencode($aParams['sc_apbgcol']) ?>&nmgp_tipo_pdf=<?php echo $aParams['sc_tp_pdf'] ?>&nmgp_parms_pdf=<?php echo $parms_pdf ?>&nmgp_graf_pdf=<?php echo $graf_pdf ?>"></iframe>
<?php
}
else
{
?>
<iframe name="nmIfrPdf" height="1px" width="1px" frameborder="0" scrolling="no" allowtransparency="true" src="Motel_Top_Hours.php?nmgp_opcao=pdf_res&script_case_init=<?php echo $aParams['script_case_init'] ?>&script_case_session=<?php echo session_id() ?>&pbfile=<?php echo $aParams['pbfile'] ?>&pdf_base=<?php echo $NM_pdfbase ?>&pdf_url=<?php echo $NM_pdfurl ?>&sc_apbgcol=<?php echo urlencode($aParams['sc_apbgcol']) ?>&nmgp_tipo_pdf=<?php echo $aParams['sc_tp_pdf'] ?>&nmgp_parms_pdf=<?php echo $parms_pdf ?>&nmgp_graf_pdf=<?php echo $graf_pdf ?>"></iframe>
<?php
}
?>
   </td>
